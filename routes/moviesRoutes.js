...
router.get('/search',
  authenticateJwt,
  checkRoles(['admin']),
  validatorHandler(searchMovieSchema, 'query'),
  async (req, res, next) => {
    try {
      ...
      const results = await moviesService.searchMovieByName(title);
...
// Ruta para subir una película
router.post('/upload',
  authenticateJwt,
  checkRoles(['admin']),
  multiUpload,
  completeInfoUser,
  validatorHandler(uploadMovieSchema, 'body'),
  async (req, res, next) => {
    try {
...
async function processFile(taskId, fileInfo) {
  try {
    ...
    await moviesService.uploadMovie(fileInfo, (progress) => {
